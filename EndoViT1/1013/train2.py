...
class FeatureDataset(Dataset):
    ...
    def _get_label(self, fname):
        # A+C → 0, B → 1
        if fname.startswith("A") or fname.startswith("C"):
            return 0
...
class_weights = torch.tensor([1.0, 1.0], dtype=torch.float).to(device)  # 可自行調整

# ======== Output Dir ========
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
save_dir = f"results/run2_binary_{timestamp}"
...
for epoch in range(num_epochs):
    ...
    y_true = torch.tensor(trues)
    y_score = all_logits[:, 1].numpy()
    map50 = average_precision_score(y_true, y_score)
...
# ===== Save Model =====
torch.save(model.state_dict(), os.path.join(save_dir, "mlp_classifier_bn_binary.pt"))
...
plt.plot(r, p, label=f"AP={map50:.3f}")
plt.xlabel("Recall")
