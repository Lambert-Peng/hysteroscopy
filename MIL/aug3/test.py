...
SPEC_TARGET = 0.90
SELECT_BY = "macro_f1"   # options: "macro_f1", "abnormal_recall", "accuracy"
...
# =============================================================================
# (Part 1) Inference export: Binary gate + Tri-class (M1 style)
# =============================================================================
def run_inference_export(save_dir: str) -> str:
    model_bin = load_model(BINARY_MODEL_PATH, num_classes=2)  # output: [Abnormal(A+C), B]
    model_tri = load_model(TRICLASS_MODEL_PATH, num_classes=3) # output: [A, B, C]
    ...
    with torch.no_grad():
        for features, label, filenames in loader:
            ...
            p_abn = probs_bin[:, 0] # P(Abnormal)
            
            # --- (2) Tri model -> for conditional A/C inside abnormal ---
            logits_tri = model_tri(features)
            probs_tri = F.softmax(logits_tri, dim=1).detach().cpu().numpy().astype(np.float32)
            pA_tri, pB_tri, pC_tri = probs_tri[:, 0], probs_tri[:, 1], probs_tri[:, 2]

            # Calculate conditional probs P(A|Abn) and P(C|Abn) from the tri-class head
            sum_AC = (pA_tri + pC_tri + 1e-8).astype(np.float32)
            p_A_cond = (pA_tri / sum_AC).astype(np.float32)
            p_C_cond = (pC_tri / sum_AC).astype(np.float32)

            # --- (3) Compose tri-class soft probs (Matches M2 output format) ---
            ...
            for i in range(len(filenames)):
                ...
                results.append({
                    ...
                    # soft outputs for MIL (unified naming with M2)
                    "p_abn": float(p_abn[i]),
...
def mil_predict_one_bag(df_bag: pd.DataFrame, pooling: str, thr: float):
    # Uses the same logic as M2, just ensuring inputs are present
...
def bag_triclass_scores(df_bag: pd.DataFrame, pooling: str):
    """
    Unified with M2: Read the pre-calculated prob_A/B/C from Excel.
    """
    p_abn = df_bag["p_abn"].to_numpy(float)
...
def plot_macro_average_roc_overlay(df: pd.DataFrame, save_dir: str):
    ...
    out_path = os.path.join(save_dir, "M1_ROC.png") # Kept as M1
...
def main():
    ...
    print("=" * 80)
    print("[STEP 1] Export combined instance predictions (Binary gate + Tri-class => 3-class probs)")
    ...
    print("[DONE]")
    print("Outputs:")
    print(" - combined_predictions.xlsx")
    print(" - MIL_sweep_spec.xlsx")
    print(" - M1_ROC.png")
