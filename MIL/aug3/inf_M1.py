# run_binarytri_MIL_sweep_and_macroROC.py
# ------------------------------------------------------------
# What you get (ONE run):
# 1) Inference export (binary + tri):
#    - Binary model: p_abn_bin = P(A+C | x)
#    - Tri model:   pA_cond_tri, pC_cond_tri (conditional within A/C)
# 2) MIL bag-level evaluation (binary+tri only):
#    - pooling: max / mean / top3
#    - sweep threshold T_abn (bag-level, for A+C vs B)
#    - pick best T under constraint: abnormal_specificity >= SPEC_TARGET
#    - save: Excel (summary + sweep tables + best per-class metrics) + CM/Metrics/Sweep plots
# 3) Macro-average ROC overlay (3-class) in ONE figure:
#    - uses inference3.py macro-average ROC method (union FPR + interp mean TPR)
#    - overlays 3 curves: max / mean / top3
#
# NOTE:
# - This script intentionally REMOVES the other two ROC plots (abnormal-vs-B ROC and OVR ROC).
# - Tri-class scores for ROC are built from the combined model outputs:
#     pA = p_abn_bin * pA_cond_tri
#     pC = p_abn_bin * pC_cond_tri
#     pB = 1 - p_abn_bin
# ------------------------------------------------------------

import os
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from datetime import datetime
from pathlib import Path
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader
import torch_directml
from sklearn.metrics import (
    confusion_matrix, ConfusionMatrixDisplay,
    accuracy_score, f1_score, precision_score, recall_score,
    roc_curve, auc
)
from sklearn.preprocessing import label_binarize

BINARY_MODEL_PATH   = "1210/results/best/train2_1layer_mean_20251210_010556/best_model_binary.pt"
TRICLASS_MODEL_PATH = "1210/results/best/train3_1layer_mean_20251210_021143/best_model_3class.pt"
VAL_DATA_DIR        = "dataset/val/weights"

THR_START = 0.00
THR_END   = 1.00
THR_STEP  = 0.01

SPEC_TARGET = 0.90
# options: "macro_f1", "abnormal_recall", "accuracy"
SELECT_BY = "macro_f1"
# =============================================================================

CLASS_NAMES = ["A", "B", "C"]
IDX_A, IDX_B, IDX_C = 0, 1, 2


# =============================================================================
# Utilities
# =============================================================================
def ensure_dir(path: str) -> None:
    os.makedirs(path, exist_ok=True)

def extract_bag_id(filename: str) -> str:
    parts = str(filename).split('_')
    if len(parts) >= 2:
        return f"{parts[0]}_{parts[1]}"
    return str(filename)

def abnormal_recall(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    y_true_bin = (y_true != IDX_B).astype(int)   # 1=abn(A/C), 0=B
    y_pred_bin = (y_pred != IDX_B).astype(int)
    TP = np.sum((y_true_bin == 1) & (y_pred_bin == 1))
    FN = np.sum((y_true_bin == 1) & (y_pred_bin == 0))
    return float(TP / (TP + FN + 1e-8))

def abnormal_specificity(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    y_true_bin = (y_true != IDX_B).astype(int)   # 1=abn(A/C), 0=B
    y_pred_bin = (y_pred != IDX_B).astype(int)
    TN = np.sum((y_true_bin == 0) & (y_pred_bin == 0))
    FP = np.sum((y_true_bin == 0) & (y_pred_bin == 1))
    return float(TN / (TN + FP + 1e-8))

def per_class_table(y_true: np.ndarray, y_pred: np.ndarray):
    """
    Return:
      - df_metrics: per-class metrics table (like your screenshot)
      - cm: 3x3 confusion matrix
    Columns:
      Class, Precision, Recall, F1, Spec, NPV, MCC, TP, FP, FN, TN
    """
    labels = [0, 1, 2]
    prec = precision_score(y_true, y_pred, labels=labels, average=None, zero_division=0)
    rec  = recall_score(y_true, y_pred, labels=labels, average=None, zero_division=0)
    f1v  = f1_score(y_true, y_pred, labels=labels, average=None, zero_division=0)

    cm = confusion_matrix(y_true, y_pred, labels=labels)

    rows = []
    for i, cname in enumerate(CLASS_NAMES):
        TP = int(cm[i, i])
        FN = int(cm[i, :].sum() - TP)
        FP = int(cm[:, i].sum() - TP)
        TN = int(cm.sum() - (TP + FN + FP))

        spec = TN / (TN + FP + 1e-8)
        npv  = TN / (TN + FN + 1e-8)

        # Matthews Correlation Coefficient (one-vs-rest for this class)
        denom = (TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)
        mcc = 0.0 if denom <= 0 else (TP * TN - FP * FN) / np.sqrt(denom)

        rows.append({
            "Class": cname,
            "Precision": float(prec[i]),
            "Recall": float(rec[i]),
            "F1": float(f1v[i]),
            "Spec": float(spec),
            "NPV": float(npv),
            "MCC": float(mcc),
            "TP": TP,
            "FP": FP,
            "FN": FN,
            "TN": TN,
        })

    df = pd.DataFrame(rows, columns=[
        "Class","Precision","Recall","F1","Spec","NPV","MCC","TP","FP","FN","TN"
    ])
    return df, cm

def save_cm_plots(cm: np.ndarray, save_dir: str, title_prefix: str, save_prefix: str):
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=CLASS_NAMES)

    # counts
    fig, ax = plt.subplots(figsize=(6, 6))
    disp.plot(ax=ax, cmap="Blues", values_format="d")
    plt.title(f"{title_prefix} (Counts)")
    plt.savefig(os.path.join(save_dir, f"{save_prefix}_counts.png"), dpi=300)
    plt.close()

    # row-normalized
    row = cm.sum(axis=1, keepdims=True)
    row[row == 0] = 1
    cmn = cm.astype(float) / row

    disp2 = ConfusionMatrixDisplay(confusion_matrix=cmn, display_labels=CLASS_NAMES)
    fig, ax = plt.subplots(figsize=(6, 6))
    disp2.plot(ax=ax, cmap="Oranges", values_format=".2f")
    plt.title(f"{title_prefix} (Percent)")
    plt.savefig(os.path.join(save_dir, f"{save_prefix}_percent.png"), dpi=300)
    plt.close()

def save_metrics_table_figure(df_metrics: pd.DataFrame, save_path: str, title: str):
    """
    Create a figure like your screenshot:
      Big title + table (Class + metrics columns)
    """
    fig, ax = plt.subplots(figsize=(18, 5))
    ax.axis("off")

    # Title
    ax.set_title(title, fontsize=26, fontweight="bold", pad=18)

    # Prepare display values (4 decimals for floats, ints for counts)
    df_disp = df_metrics.copy()
    float_cols = ["Precision","Recall","F1","Spec","NPV","MCC"]
    for c in float_cols:
        df_disp[c] = df_disp[c].map(lambda x: f"{x:.4f}")
    for c in ["TP","FP","FN","TN"]:
        df_disp[c] = df_disp[c].astype(int).astype(str)

    col_labels = list(df_disp.columns)
    cell_text  = df_disp.values.tolist()

    table = ax.table(
        cellText=cell_text,
        colLabels=col_labels,
        cellLoc="center",
        loc="center"
    )

    # Style: thicker borders + readable size
    table.auto_set_font_size(False)
    table.set_fontsize(14)
    table.scale(1.2, 2.0)

    # thicker grid lines
    for (row, col), cell in table.get_celld().items():
        cell.set_linewidth(1.8)
        if row == 0:
            cell.set_fontsize(15)
            cell.set_text_props(fontweight="bold")

    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()




def plot_sweep(sweep_df: pd.DataFrame, save_dir: str, title: str, save_name: str):
    fig, ax = plt.subplots(figsize=(8, 4.5))
    ax.plot(sweep_df["threshold"], sweep_df["macro_f1"], label="macro_f1")
    ax.plot(sweep_df["threshold"], sweep_df["accuracy"], label="accuracy")
    ax.plot(sweep_df["threshold"], sweep_df["abnormal_recall"], label="abnormal_recall")
    ax.plot(sweep_df["threshold"], sweep_df["abnormal_specificity"], label="abnormal_specificity")
    ax.axhline(SPEC_TARGET, linestyle="--", linewidth=1, label=f"spec_target={SPEC_TARGET}")
    ax.set_xlabel("T_abn (bag threshold for A+C vs B)")
    ax.set_ylabel("score")
    ax.set_title(title)
    ax.grid(True)
    ax.legend()
    plt.savefig(os.path.join(save_dir, save_name), dpi=300, bbox_inches="tight")
    plt.close()

def pick_best_threshold_with_spec(sweep_df: pd.DataFrame):
    cand = sweep_df[sweep_df["abnormal_specificity"] >= SPEC_TARGET].copy()

    if len(cand) == 0:
        tmp = sweep_df.copy()
        tmp["spec_gap"] = np.abs(tmp["abnormal_specificity"] - SPEC_TARGET)
        best = tmp.sort_values(["spec_gap", "threshold"]).iloc[0].to_dict()
        return float(best["threshold"]), best, False

    if SELECT_BY == "macro_f1":
        key = "macro_f1"
    elif SELECT_BY == "abnormal_recall":
        key = "abnormal_recall"
    elif SELECT_BY == "accuracy":
        key = "accuracy"
    else:
        raise ValueError("SELECT_BY must be macro_f1 / abnormal_recall / accuracy")

    best = cand.loc[cand[key].idxmax()].to_dict()
    return float(best["threshold"]), best, True


# =============================================================================
# Inference (binary + tri)
# =============================================================================
class RecursiveFeatureDataset(Dataset):
    def __init__(self, root_dir: str):
        self.root_dir = Path(root_dir)
        self.files = sorted(list(self.root_dir.rglob("*.pt")))
        self.labels = [self._get_label(f) for f in self.files]

    def _get_label(self, fpath: Path) -> int:
        name = fpath.name
        if name.startswith("A"): return 0
        if name.startswith("B"): return 1
        if name.startswith("C"): return 2
        return 0

    def __len__(self) -> int:
        return len(self.files)

    def __getitem__(self, idx: int):
        fpath = self.files[idx]
        feat = torch.load(fpath).float()

        # keep your original pooling behavior
        if feat.dim() == 3:
            feat = feat[0].mean(dim=0)
        elif feat.dim() == 2:
            feat = feat.mean(dim=0)
        elif feat.dim() == 1 and feat.shape[0] > 768:
            feat = feat.view(-1, 768).mean(dim=0)

        y = torch.tensor(self.labels[idx], dtype=torch.long)
        return feat, y, fpath.name

class MLPClassifier(nn.Module):
    def __init__(self, in_dim=768, num_classes=3):
        super().__init__()
        self.linear = nn.Linear(in_dim, num_classes)

    def forward(self, x):
        return self.linear(x)

def load_model(path: str, num_classes: int, device):
    print(f"[INFO] Loading: {path}")
    m = MLPClassifier(num_classes=num_classes)
    m.load_state_dict(torch.load(path, map_location=device))
    m.to(device).eval()
    return m

def run_inference_export(save_dir: str) -> str:
    device = torch_directml.device()

    model_bin = load_model(BINARY_MODEL_PATH, num_classes=2, device=device)
    model_tri = load_model(TRICLASS_MODEL_PATH, num_classes=3, device=device)

    ds = RecursiveFeatureDataset(VAL_DATA_DIR)
    loader = DataLoader(ds, batch_size=32, shuffle=False)

    rows = []
    with torch.no_grad():
        for feat, y, fnames in loader:
            feat = feat.to(device)

            # Binary: p_abn_bin (assume index 0 = Abnormal(A+C), index 1 = B)
            logits_bin = model_bin(feat)
            probs_bin = F.softmax(logits_bin, dim=1).cpu().numpy().astype(np.float32)
            p_abn_bin = probs_bin[:, 0]
            p_B_bin   = probs_bin[:, 1]

            # Tri: for conditional A/C inside abnormal
            logits_tri = model_tri(feat)
            probs_tri = F.softmax(logits_tri, dim=1).cpu().numpy().astype(np.float32)
            pA_tri, pB_tri, pC_tri = probs_tri[:, 0], probs_tri[:, 1], probs_tri[:, 2]

            sum_AC = (pA_tri + pC_tri + 1e-8).astype(np.float32)
            pA_cond_tri = (pA_tri / sum_AC).astype(np.float32)
            pC_cond_tri = (pC_tri / sum_AC).astype(np.float32)

            for i in range(len(fnames)):
                rows.append({
                    "filename": fnames[i],
                    "true_label": int(y[i].item()),
                    "p_abn": float(p_abn_bin[i]),
                    "p_B_bin": float(p_B_bin[i]),
                    "p_A_cond": float(pA_cond_tri[i]),
                    "p_C_cond": float(pC_cond_tri[i]),
                })

    df = pd.DataFrame(rows)
    out_xlsx = os.path.join(save_dir, "combined_predictions.xlsx")
    df.to_excel(out_xlsx, index=False)
    print(f"[INFO] Saved inference Excel: {out_xlsx}")
    return out_xlsx


# =============================================================================
# MIL (binary+tri only): Stage1 threshold on pooled p_abn_bin, Stage2 A vs C
# =============================================================================
def mil_predict_one_bag_binarytri(df_bag: pd.DataFrame, pooling: str, thr: float):
    p_abn = df_bag["p_abn"].to_numpy(float)
    pA_cond = df_bag["p_A_cond"].to_numpy(float)
    pC_cond = df_bag["p_C_cond"].to_numpy(float)

    n = len(p_abn)
    if n == 0:
        return IDX_B, 0.0

    if pooling == "max":
        idx = int(np.argmax(p_abn))
        I = [idx]
        S = float(np.max(p_abn))
    elif pooling == "mean":
        I = list(range(n))
        S = float(np.mean(p_abn))
    elif pooling == "top3":
        order = np.argsort(-p_abn)
        k = min(3, n)
        I = order[:k].tolist()
        S = float(np.mean(p_abn[I])) if k > 0 else 0.0
    else:
        raise ValueError("pooling must be max/mean/top3")

    # Stage1: A+C vs B
    if S < thr:
        return IDX_B, S

    # Stage2: A vs C (weighted by p_abn)
    A_score = float(np.sum(p_abn[I] * pA_cond[I]))
    C_score = float(np.sum(p_abn[I] * pC_cond[I]))
    pred = IDX_A if A_score >= C_score else IDX_C
    return pred, S

def evaluate_binarytri(df: pd.DataFrame, bag_true: pd.Series, pooling: str, thr: float):
    y_true, y_pred = [], []
    for bag_id, df_bag in df.groupby("bag_id"):
        pred, _ = mil_predict_one_bag_binarytri(df_bag, pooling, thr)
        y_pred.append(pred)
        y_true.append(int(bag_true.loc[bag_id]))

    y_true = np.array(y_true, dtype=int)
    y_pred = np.array(y_pred, dtype=int)

    acc = float(accuracy_score(y_true, y_pred))
    macro_f1 = float(f1_score(y_true, y_pred, average="macro", labels=[0, 1, 2], zero_division=0))
    abn_rec = abnormal_recall(y_true, y_pred)
    abn_spec = abnormal_specificity(y_true, y_pred)
    return y_true, y_pred, acc, macro_f1, abn_rec, abn_spec


# =============================================================================
# Macro-average ROC overlay (inference3 method)
# =============================================================================
def macro_average_roc_curve_inference3(y_true_3class: np.ndarray, y_score_3class: np.ndarray, n_classes: int = 3):
    """
    inference3-style macro-average ROC:
      - label_binarize
      - roc_curve per class
      - union all FPR + interpolate & average TPR
      - AUC on macro curve
    """
    y_true_bin = label_binarize(y_true_3class, classes=list(range(n_classes)))

    all_fpr = np.unique(np.concatenate([
        roc_curve(y_true_bin[:, i], y_score_3class[:, i])[0] for i in range(n_classes)
    ]))
    mean_tpr = np.zeros_like(all_fpr)

    for i in range(n_classes):
        fpr, tpr, _ = roc_curve(y_true_bin[:, i], y_score_3class[:, i])
        mean_tpr += np.interp(all_fpr, fpr, tpr)

    mean_tpr /= n_classes
    macro_auc = auc(all_fpr, mean_tpr)
    return all_fpr, mean_tpr, float(macro_auc)

def bag_scores_triclass_from_binarytri(df_bag: pd.DataFrame, pooling: str):
    """
    Build instance tri-probs from combined outputs:
      pA = p_abn_bin * pA_cond_tri
      pC = p_abn_bin * pC_cond_tri
      pB = 1 - p_abn_bin

    Then pool to bag scores (sA, sB, sC).
    For top3, select by highest p_abn_bin (most suspicious).
    """
    p_abn = df_bag["p_abn"].to_numpy(float)
    pA_cond = df_bag["p_A_cond"].to_numpy(float)
    pC_cond = df_bag["p_C_cond"].to_numpy(float)

    pA = p_abn * pA_cond
    pC = p_abn * pC_cond
    pB = 1.0 - p_abn

    n = len(p_abn)
    if n == 0:
        return 0.0, 0.0, 0.0

    if pooling == "max":
        idx = int(np.argmax(p_abn))
        I = [idx]
    elif pooling == "mean":
        I = list(range(n))
    elif pooling == "top3":
        order = np.argsort(-p_abn)
        k = min(3, n)
        I = order[:k].tolist()
    else:
        raise ValueError("pooling must be max/mean/top3")

    sA = float(np.mean(pA[I])) if len(I) else 0.0
    sB = float(np.mean(pB[I])) if len(I) else 0.0
    sC = float(np.mean(pC[I])) if len(I) else 0.0
    return sA, sB, sC

def plot_macro_average_roc_overlay(df: pd.DataFrame, save_dir: str):
    """
    One figure: macro-average ROC overlay for 3 pooling methods (max/mean/top3).
    Macro-average ROC uses inference3-style method.
    """
    bag_true = df.groupby("bag_id")["true_label"].agg(lambda x: int(x.value_counts().idxmax()))

    fig, ax = plt.subplots(figsize=(8, 6))

    for pooling in ["max", "mean", "top3"]:
        y_true = []
        y_score = []

        for bag_id, df_bag in df.groupby("bag_id"):
            y_true.append(int(bag_true.loc[bag_id]))
            sA, sB, sC = bag_scores_triclass_from_binarytri(df_bag, pooling)
            y_score.append([sA, sB, sC])

        y_true = np.array(y_true, dtype=int)
        y_score = np.array(y_score, dtype=float)

        if len(np.unique(y_true)) < 2:
            ax.text(0.05, 0.05, f"ROC undefined (only one class present) for {pooling}", transform=ax.transAxes)
            continue

        fpr_m, tpr_m, auc_m = macro_average_roc_curve_inference3(y_true, y_score, n_classes=3)
        ax.plot(fpr_m, tpr_m, lw=2, label=f"{pooling} (Macro AUC={auc_m:.3f})")

    ax.plot([0, 1], [0, 1], lw=1, linestyle="--", label="random")
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    ax.set_title("3-class Macro-Average ROC Overlay (max / mean / top3)")
    ax.grid(True)
    ax.legend(loc="lower right")

    out_path = os.path.join(save_dir, "M1_ROC.png")
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"[INFO] Saved macro-average ROC overlay: {out_path}")


# =============================================================================
# Runner: MIL sweep + save reports + macro-average ROC overlay
# =============================================================================
def run_mil_binarytri_and_reports(infer_xlsx: str, save_dir: str):
    df = pd.read_excel(infer_xlsx)
    required = {"filename", "true_label", "p_abn", "p_A_cond", "p_C_cond"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Missing columns in inference Excel: {missing}")

    df["bag_id"] = df["filename"].apply(extract_bag_id)
    bag_true = df.groupby("bag_id")["true_label"].agg(lambda x: int(x.value_counts().idxmax()))

    thresholds = np.round(np.arange(THR_START, THR_END + 1e-12, THR_STEP), 6)

    out_xlsx = os.path.join(save_dir, "MIL_sweep_spec.xlsx")
    writer = pd.ExcelWriter(out_xlsx, engine="openpyxl")

    summary_rows = []

    for pooling in ["max", "mean", "top3"]:
        sweep_rows = []
        for thr in thresholds:
            y_true, y_pred, acc, macro_f1, abn_rec, abn_spec = evaluate_binarytri(df, bag_true, pooling, float(thr))
            sweep_rows.append({
                "threshold": float(thr),
                "accuracy": acc,
                "macro_f1": macro_f1,
                "abnormal_recall": abn_rec,
                "abnormal_specificity": abn_spec
            })

        sweep_df = pd.DataFrame(sweep_rows)
        best_thr, best_row, meets = pick_best_threshold_with_spec(sweep_df)

        sweep_sheet = f"sweep_{pooling}"[:31]
        sweep_df.to_excel(writer, sheet_name=sweep_sheet, index=False)
        plot_sweep(
            sweep_df,
            save_dir,
            title=f"{pooling} (spec>={SPEC_TARGET}, select_by={SELECT_BY})",
            save_name=f"Sweep_{pooling}.png"
        )

        y_true, y_pred, acc, macro_f1, abn_rec, abn_spec = evaluate_binarytri(df, bag_true, pooling, best_thr)
        df_metrics, cm = per_class_table(y_true, y_pred)

        best_sheet = f"best_{pooling}"[:31]
        df_metrics.to_excel(writer, sheet_name=best_sheet, index=False)

        save_cm_plots(cm, save_dir, f"{pooling} (best T={best_thr:.2f})", f"CM_{pooling}")
        # === NEW: save metrics table figure (like your screenshot) ===
        title = f"{pooling.capitalize()} Metrics"
        fig_path = os.path.join(save_dir, f"MIL_Metrics_Viz_{pooling.capitalize()}.png")
        save_metrics_table_figure(df_metrics, fig_path, title)


        summary_rows.append({
            "pooling": pooling,
            "SPEC_TARGET": SPEC_TARGET,
            "meets_specificity": bool(meets),
            "select_by": SELECT_BY,
            "best_T_abn": float(best_thr),
            "best_accuracy": float(acc),
            "best_macro_f1": float(macro_f1),
            "best_abnormal_recall": float(abn_rec),
            "best_abnormal_specificity": float(abn_spec),
        })

    pd.DataFrame(summary_rows).to_excel(writer, sheet_name="summary", index=False)
    writer.close()
    print(f"[INFO] Saved MIL report Excel: {out_xlsx}")

    # Only keep this ROC plot (macro-average overlay)
    plot_macro_average_roc_overlay(df, save_dir)


# =============================================================================
# MAIN
# =============================================================================
def main():
    run_tag = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_dir = os.path.join(os.path.dirname("results/best_model_AC.pt"), f"M1_{run_tag}")
    ensure_dir(save_dir)

    print("=" * 80)
    print("[STEP 1] Inference export (binary + tri) -> instance-level Excel")
    infer_xlsx = run_inference_export(save_dir)

    print("=" * 80)
    print("[STEP 2] MIL sweep + reports + 3-class macro-average ROC overlay")
    run_mil_binarytri_and_reports(infer_xlsx, save_dir)

    print("=" * 80)
    print(f"[DONE] All results saved in: {save_dir}")
    print(" - combined_predictions_forMIL_binary_tri.xlsx")
    print(" - MIL_binarytri_sweep_spec.xlsx")
    print(" - Sweep_*.png / CM_*.png / Metrics_*.png")
    print(" - ROC_macro_average_overlay_max_mean_top3.png")

if __name__ == "__main__":
    main()
