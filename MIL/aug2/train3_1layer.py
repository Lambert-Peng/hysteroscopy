...
class MILDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for f in files:
            ...
            feature = torch.load(fpath)   # [tokens, 768]
...
save_dir = f"results/train3_1layer_MIL_run_{timestamp}"
...
for epoch in range(num_epochs):
    ...
    for x, y, _ in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
        x, y = x.to(device), y.to(device)        # x: [1, N, 768]
        optimizer.zero_grad()
        logits, attn = model(x)
